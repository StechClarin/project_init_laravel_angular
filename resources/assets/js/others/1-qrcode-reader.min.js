(function ($, window, document, undefined) {
    ...
    QRCodeReader.prototype = {
        constructor: QRCodeReader, init: function () {
            qrr.buildHTML();
            ...
            qrr.okBtn = $('<a id="qrr-ok" class="btn shadow btn-transition btn-template-outline-2"><i class="fa fa-qrcode"></i> Valider</a>');
            qrr.loadingMessage = $('<div id="qrr-loading-message">🎥 Impossible d\'accéder au flux vidéo (veuillez vous assurer que votre webcam est activée)</div>');
            ...
            qrr.outputNoData = $('<div id="qrr-nodata">Aucun QR code détecté.</div>');
        ...
        }, doneReading: function ()
        {
            ...
            if (qrr.settings.target) {
                ...
                $(qrr.settings.target).val(value).change()
            }
            ...
            if (qrr.video.readyState === qrr.video.HAVE_ENOUGH_DATA) {
                ...
                qrr.canvas[0].height = $.qrCodeReader.height;
                qrr.canvas[0].width = $.qrCodeReader.width;
