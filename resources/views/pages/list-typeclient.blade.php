            ...
            <div class="d-inline-flex align-items-center justify-content-between gap-2 w-100 p-10 px-0">
                <div class="titre-ch-p">
                    <div class="card-title d-flex align-self-center mb-0 me-3">
                        ...
                        <span class="badge badge-primary p-3"> @{{paginations['typeclient'].totalItems | currency:"":0 | convertMontant}}</span>
                    </div>
                </div>
                <div class="dropdown dropdown-inline" title="{{ __('customlang.ajouter') }}" data-bs-toggle="tooltip"
                    ...
                    <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-title-gray-700 overtop-filterbar menu-icon-muted menu-active-bg menu-state-color fw-semibold py-4 fs-base w-175px"
                        ...
                        <div class="menu-item px-3 my-0" ng-click="showModalAdd('typeclient', {is_file_excel:true, title: 'Type d\'activités'})">
                            <a href="" class="menu-link px-3 py-2">
                                <span class="menu-icon" data-kt-element="icon">
                                    <span class="svg-icon svg-icon-3">
                        ...
                        <div class="menu-item px-3 my-0" title="Télécharger un fichier excel modèle">
                            <a href="typeclient.feuille" class="menu-link px-3 py-2">
                                <span class="menu-icon" data-kt-element="icon">
                                    <span class="svg-icon svg-icon-3">
                                        {!! file_get_contents(public_path('assets/media/svg/icons/sidebar/icon-item.svg')) !!}
                                    </span>
                                    ...
                                    <form ng-submit="pageChanged('typeclient')">
                                        <div class="form-row row animated fadeIn mt-delete">
                                            <div class="col-md-12 form-group">
                                                <input type="text" class="form-control" id="search_list_typeclient"
                                                    ng-model="search_list_typeclient"
                                                    placeholder="Rechercher par nom ..."
                                                    ng-model-options="{ debounce: 500 }"
                                                    ng-change="pageChanged('typeclient')">
                                            </div>
                                        </div>
                                        <div class="w-100 text-center pb-4">
                                            <button type="button" class="me-2 btn shadow btn-transition btn-danger float-start" ng-click="pageChanged('typeclient', {justWriteUrl : 'typeclients-pdf'})">
                                                <span class="d-md-block d-none pr-2 pl-2">{{__('customlang.pdf')}}</span>
                                                <i class="fa fa-file-pdf"></i>
                                            </button>
                                            <button type="button" class="btn shadow btn-transition btn-success float-start" ng-click="pageChanged('typeclient', {justWriteUrl : 'typeclients-excel'})">
                                    ...
                                    <table class="m-auto table-striped table table-head-custom table-vertical-center table-head-bg table-borderless table-report text-center">
                                        <thead>
                                            <tr>
                                                ...
                                                @if(auth()->user()->can('suppression-typeclient') || auth()->user()->can('modification-typeclient') || auth()->user()->can('creation-typeclient'))
                                                ...
                                                @if(auth()->user()->can('suppression-typeclient') || auth()->user()->can('modification-typeclient') || auth()->user()->can('creation-typeclient'))
                                                    <td class="pr-0 text-right">
                                                        <div class="menu-leftToRight d-flex align-items-center justify-content-center">
                                                            ...
                                                            @if(auth()->user()->can('suppression-typeclient'))
                                                            ...
                                                            @if(auth()->user()->can('modification-typeclient'))
                                                                <button class="menu-btn-item btn btn-sm btn-warning btn-icon font-size-sm" ng-click="showModalUpdate('typeclient', item.id, 'null', 'null')" title="{{__('customlang.modifier')}}">
                                                                    <i class="flaticon2-edit"></i>
                                                                </button>
                                                            @endif
                                                            @if(auth()->user()->can('creation-typeclient'))
                                                                <button class="menu-btn-item btn btn-sm btn-light-primary btn-icon font-size-sm" ng-click="showModalUpdate('typeclient',item.id,{forceChangeForm: false, isClone:true}, 'null')" title="{{ __('customlang.cloner') }}">
