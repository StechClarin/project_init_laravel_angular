            ...
            <div class="d-inline-flex align-items-center justify-content-between gap-2 w-100 p-10 px-0">
                <div class="titre-ch-p">
                    <div class="card-title d-flex align-self-center mb-0 me-3">
                        ...
                        <span class="badge badge-primary p-3"> @{{paginations['newsletter'].totalItems +  paginations['newsletter']['newsletter_homeostasia'].totalItems | currency:"":0 | convertMontant}}</span>
                    </div>
                </div>
                {{-- <div class="dropdown dropdown-inline" title="{{ __('customlang.ajouter') }}" data-bs-toggle="tooltip"
                    ...
                    @if(auth()->user()->can('creation-newsletter') || auth()->user()->can('modification-newsletter'))
                    ...
                    <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-title-gray-700 overtop-filterbar menu-icon-muted menu-active-bg menu-state-color fw-semibold py-4 fs-base w-175px"
                        ...
                        @if(auth()->user()->can('creation-newsletter'))
                        <div class="menu-item px-3 my-0" ng-click="showModalAdd('newsletter')">
                        ...
                        <div class="menu-item px-3 my-0" ng-click="showModalAdd('newsletter', {is_file_excel:true, title: 'Type d\'activités'})">
                            <a href="" class="menu-link px-3 py-2">
                                <span class="menu-icon" data-kt-element="icon">
                                    <span class="svg-icon svg-icon-3">
                        ...
                        <div class="menu-item px-3 my-0" title="Télécharger un fichier excel modèle">
                            <a href="newsletter.feuille" class="menu-link px-3 py-2">
                                <span class="menu-icon" data-kt-element="icon">
                                    <span class="svg-icon svg-icon-3">
                                        {!! file_get_contents(public_path('assets/media/svg/icons/sidebar/icon-item.svg')) !!}
                                    </span>
                    ...
                    <div
                        ...
                        <div class="card rounded-1">
                            ...
                            <div id="filtres" class="card collapse">
                                <div class="card-body">
                                    <form ng-submit="pageChanged('newsletter')">
                                        <div class="form-row row animated fadeIn mt-delete">
                                            <div class="col-md-12 form-group">
                                                <input type="text" class="form-control" id="search_list_newsletter"
                                                    ng-model="search_list_newsletter"
                                                    placeholder="Rechercher par mail ..."
                                                    ng-model-options="{ debounce: 500 }"
                                                    ng-change="pageChanged('newsletter')">
                                            </div>
                                        </div>
                                        <div class="w-100 text-center pb-4">
                                            <button type="button" class="me-2 btn shadow btn-transition btn-danger float-start" ng-click="pageChanged('newsletter', {justWriteUrl : 'newsletters-pdf'})">
                                                <span class="d-md-block d-none pr-2 pl-2">{{__('customlang.pdf')}}</span>
                                                <i class="fa fa-file-pdf"></i>
                                            </button>
                                            <button type="button" class="btn shadow btn-transition btn-success float-start" ng-click="pageChanged('newsletter', {justWriteUrl : 'newsletters-excel'})">
                                            ...
                                            <button type="reset" class="me-2 btn shadow-sm btn-transition btn-light-dark float-end" ng-click="emptyForm('newsletter', true)">
                                    ...
                                    <table class="m-auto table-striped table table-head-custom table-vertical-center table-head-bg table-borderless table-report text-center">
                                        <thead>
                                            <tr class="bg-primary text-white">
                                                ...
                                                @if(auth()->user()->can('suppression-newsletter') || auth()->user()->can('modification-newsletter') || auth()->user()->can('creation-newsletter'))
                                        ...
                                        <tbody>
                                            <tr  ng-repeat="item in dataPage['newsletters']">
                                                ...
                                                @if(auth()->user()->can('suppression-newsletter') || auth()->user()->can('modification-newsletter') || auth()->user()->can('creation-newsletter'))
                                                    <td class="pr-0 text-right">
                                                        <div class="menu-leftToRight d-flex align-items-center justify-content-center">
                                                            ...
                                                            @if(auth()->user()->can('suppression-newsletter'))
                                                                <button class="menu-btn-item btn btn-sm btn-danger btn-icon font-size-sm" ng-click="deleteElement('newsletter', item.id)" title="{{__('customlang.supprimer')}}">
                                                                    <i class="flaticon2-trash"></i>
                                                                </button>
                                                            @endif
                                                            @if(auth()->user()->can('modification-newsletter'))
                                                                <button class="menu-btn-item btn btn-sm btn-warning btn-icon font-size-sm" ng-click="showModalUpdate('newsletter', item.id, 'null', 'null')" title="{{__('customlang.modifier')}}">
                                                                    <i class="flaticon2-edit"></i>
                                                                </button>
                                                            @endif
                                                            @if(auth()->user()->can('creation-newsletter'))
                                                                <button class="menu-btn-item btn btn-sm btn-light-primary btn-icon font-size-sm" ng-click="showModalUpdate('newsletter',item.id,{forceChangeForm: false, isClone:true}, 'null')" title="{{ __('customlang.cloner') }}">
                    ...
                    <div class="card p-5 rounded-1">
                        <div class="d-flex justify-content-between align-items-center flex-wrap">
                            <div class="d-flex align-items-center me-3">
                                ...
                                <select class="form-control form-control-sm text-primary fw-bold me-4 border-0 bg-light-primary" style="width: 75px;" ng-model="paginations['newsletter'].entryLimit" ng-change="pageChanged('newsletter')">
                            ...
                            <div class="d-flex flex-wrap">
                                <nav aria-label="...">
                                    <ul class="pagination float-md-end justify-content-center mt-1" uib-pagination total-items="paginations['newsletter'].totalItems" ng-model="paginations['newsletter'].currentPage" max-size="paginations['newsletter'].maxSize" items-per-page="paginations['newsletter'].entryLimit" ng-change="pageChanged('newsletter')" previous-text="‹" next-text="›" first-text="«" last-text="»" boundary-link-numbers="true" rotate="false"></ul>
                                </nav>
                            </div>
                        </div>
                    </div>
