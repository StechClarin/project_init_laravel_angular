        ...
        <div class="container d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
            <div class="d-flex align-items-center me-1">
                <div class="d-flex align-items-baseline flex-wrap me-5 nav-text">
                        ...
                        <div class="card-title d-flex align-self-center mb-0 me-3">
                            <span class="card-icon align-self-center">
                                <span class="svg-icon svg-icon-primary">
                                    {!! file_get_contents(public_path('assets/media/svg/icons/sidebar/icon-cotation.svg')) !!}
                                </span>
                            </span>
                    ...
                    <span class="badge badge-light-primary">@{{paginations['typeexpedition'].totalItems | currency:"":0 | convertMontant}}</span>
                </div>
            </div>
            <div class="d-flex align-items-center flex-wrap">
                <div class="dropdown dropdown-inline" data-toggle="tooltip" title="{{ __('customlang.ajouter') }}" data-placement="left">
                    @if(auth()->user()->can('creation-typeexpedition') || auth()->user()->can('modification-typeexpedition'))
                        ...
                        <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-title-gray-700 menu-icon-muted menu-active-bg menu-state-color fw-semibold py-4 fs-base w-175px" data-kt-menu="true" data-kt-element="theme-mode-menu">
                            <ul class="navi navi navi-hover text-center">
                                @if(auth()->user()->can('creation-typeexpedition'))
                                    <div class="menu-item px-3 my-0" ng-click="showModalAdd('typeexpedition')">
                            ...
                            @endif
                                @if(auth()->user()->can('modification-typeexpedition'))
                                    <div class="menu-item px-3 my-0" ng-click="showModalAdd('typeexpedition', {is_file_excel:true, title: 'Mode de paiement'})">
                                        <a href="" class="menu-link px-3 py-2">
                                        <span class="menu-icon" data-kt-element="icon">
                                            <span class="svg-icon svg-icon-3">
                    ...
                    <div class="card card-custom mb-10 accordion accordion-solid accordion-panel accordion-svg-toggle cursor-pointer">
                        <div class="card">
                            ...
                            <div id="filtres" class="card collapse">
                                <div class="card-body">
                                    <form ng-submit="pageChanged('typeexpedition')">
                                        <div class="form-row animated fadeIn mt-4">
                                            <div class="col-md-12 form-group">
                                                <input type="text" class="form-control" id="nom_list_typeexpedition" ng-model="nom_list_typeexpedition" placeholder="{{ __('customlang.rechercher_par_nom') }}" ng-model-options="{ debounce: 500 }" ng-change="pageChanged('typeexpedition')">
                                            </div>
                                        </div>
                                        <div class="w-100 text-center pb-4">
                                            <button type="button" class="me-2 btn shadow-sm btn-transition btn-danger float-start" ng-click="pageChanged('typeexpedition', {justWriteUrl : 'typeexpeditions-pdf'})">
                                                <span class="d-md-block d-none pr-2 pl-2">{{__('customlang.pdf')}}</span>
                                                <i class="fa fa-file-pdf"></i>
                                            </button>
                                            <button type="button" class="btn shadow-sm btn-transition btn-success float-start" ng-click="pageChanged('typeexpedition', {justWriteUrl : 'typeexpeditions-excel'})">
                                            ...
                                            <button type="reset" class="me-2 btn shadow-sm btn-transition btn-light-dark float-end" ng-click="emptyForm('typeexpedition', true)">
                                    ...
                                    <table class="table table-head-custom table-vertical-center table-head-bg table-borderless table-report text-center">
                                        <thead>
                                            <tr>
                                                ...
                                                {{-- <th style="min-width: 120px">typeexpedition</th> --}}
                                        ...
                                        <tbody>
                                            <tr class="" ng-repeat="item in dataPage['typeexpeditions']">
                                                <td>
                                                ...
                                                {{-- <td>
                                                    <span class="badge badge-@{{item.color_status_is_typeexpedition}}" >@{{item.is_typeexpedition_fr}}</span>
                                                </td> --}}
                                                <td class="pr-0 text-right">
                                                    <div class="menu-leftToRight d-flex align-items-center justify-content-center">
                                                        ...
                                                        @if(auth()->user()->can('suppression-typeexpedition'))
                                                            <button class="menu-btn-item btn btn-sm btn-danger btn-icon font-size-sm" ng-click="deleteElement('typeexpedition', item.id)" title="{{__('customlang.supprimer')}}">
                                                                <i class="flaticon2-trash"></i>
                                                            </button>
                                                        @endif
                                                        @if(auth()->user()->can('modification-typeexpedition'))
                                                            <button class="menu-btn-item btn btn-sm btn-warning btn-icon font-size-sm"  ng-click="showModalUpdate('typeexpedition', item.id, 'null', 'null')" title="{{__('customlang.modifier')}}">
                                                                <i class="flaticon2-edit"></i>
                                                            </button>
                                                        @endif
                                                        @if(auth()->user()->can('cloner-typeexpedition'))
                                                            <button class="menu-btn-item btn btn-sm btn-primary btn-icon font-size-sm" ng-click="showModalUpdate('typeexpedition',item.id,{forceChangeForm: false, isClone:true}, 'null')" title="cloner un type d'expedition">
...
<div class="footer py-4 d-flex flex-lg-column bg-body" id="kt_footer">
    <div class="container d-flex justify-content-between align-items-center flex-wrap">
        <div class="d-flex align-items-center me-3">
            ...
            <select class="form-control form-control-sm text-primary fw-bold me-4 border-0 bg-light-primary" style="width: 75px;" ng-model="paginations['typeexpedition'].entryLimit" ng-change="pageChanged('typeexpedition')">
        ...
        <div class="d-flex flex-wrap">
            <nav aria-label="...">
                <ul class="pagination float-md-end justify-content-center mt-1" uib-pagination total-items="paginations['typeexpedition'].totalItems"
                ng-model="paginations['typeexpedition'].currentPage" max-size="paginations['typeexpedition'].maxSize" items-per-page="paginations['typeexpedition'].entryLimit"
                ng-change="pageChanged('typeexpedition')" previous-text="‹" next-text="›" first-text="«" last-text="»" boundary-link-numbers="true" rotate="false"></ul>
            </nav>
        </div>
    </div>
</div>
