            ...
            <div class="d-inline-flex align-items-center justify-content-between gap-2 w-100 p-10 px-0">
                <div class="titre-ch-p">
                    <div class="card-title d-flex align-self-center mb-0 me-3">
                        ...
                        <span class="badge badge-primary p-3"> @{{paginations['client'].totalItems | currency:"":0 | convertMontant}}</span>
                    </div>
                </div>
                <div class="dropdown dropdown-inline" title="{{ __('customlang.ajouter') }}" data-bs-toggle="tooltip"
                    ...
                    @if(auth()->user()->can('creation-client') || auth()->user()->can('modification-client'))
                    ...
                    <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-title-gray-700 overtop-filterbar menu-icon-muted menu-active-bg menu-state-color fw-semibold py-4 fs-base w-175px"
                        ...
                        @if(auth()->user()->can('creation-client'))
                        <div class="menu-item px-3 my-0" ng-click="showModalAdd('client')">
                        ...
                        <div class="menu-item px-3 my-0" ng-click="showModalAdd('client', {is_file_excel:true, title: 'importation client'})">
                            <a href="" class="menu-link px-3 py-2">
                                <span class="menu-icon" data-kt-element="icon">
                                    <span class="svg-icon svg-icon-3">
                        ...
                        <div class="menu-item px-3 my-0" title="Télécharger un fichier excel modèle">
                            <a href="client.feuille" class="menu-link px-3 py-2">
                                <span class="menu-icon" data-kt-element="icon">
                                    <span class="svg-icon svg-icon-3">
                                        {!! file_get_contents(public_path('assets/media/svg/icons/sidebar/icon-item.svg')) !!}
                                    </span>
                    ...
                    <div
                        ...
                        <div class="card rounded-1">
                            ...
                            <div id="filtres" class="card collapse">
                                <div class="card-body">
                                    <form ng-submit="pageChanged('client')">
                                        <div class="form-row row animated fadeIn mt-delete">
                                            <div class="col-md-12 form-group">
                                                <div class="d-flex flex-column mb-8 fv-row">
                                                    ...
                                                    <select id="type_client_list_client"
                                                        class="form-control form-control-solid select2 modalselect2"
                                                        style="width: 100% !important;">
                                                        <option value="">--</option>
                                                        <option value="@{{ item.id }}"
                                                            ng-repeat="item in dataPage['typeclients']">
                                                            @{{ item.nom }}</option>
                                                    </select>
                                                </div>
                                            </div>
                                            <div class="col-md-12 form-group">
                                                <input type="text" class="form-control" id="search_list_client"
                                                    ng-model="search_list_client"
                                                    placeholder="Rechercher par  nom, ..."
                                                    ng-model-options="{ debounce: 500 }"
                                                    ng-change="pageChanged('client')">
                                            </div>
                                                    ...
                                                    <div class="input-group">
                                                        <div class="form-check form-check-custom form-check-solid me-5">
                                                            <input class="form-check-input" type="radio"
                                                                id="status_oui_list_client"
                                                                name="status"
                                                                data-value="true"
                                                                autocomplete="off"
                                                                for="status_oui_list_client" />
                                                            <label class="form-check-label" for="status_oui_client">
                                                                Oui
                                                            </label>
                                                        </div>
                                                        <div class="form-check form-check-custom form-check-solid">
                                                            <input class="form-check-input" type="radio"
                                                                id="status_non_list_client"
                                                                name="status"
                                                                ng-model="status"
                                                                data-value="false"
                                                                autocomplete="off"
                                                                for="status_non_list_client" />
                                                            <label class="form-check-label" for="status_non_client">
                                                                Non
                                                            </label>
                                                        </div>
                                                    </div>
                                        ...
                                        <div class="w-100 text-center pb-4">
                                            <button type="button" class="me-2 btn shadow btn-transition btn-danger float-start" ng-click="pageChanged('client', {justWriteUrl : 'clients-pdf'})">
                                                <span class="d-md-block d-none pr-2 pl-2">{{__('customlang.pdf')}}</span>
                                                <i class="fa fa-file-pdf"></i>
                                            </button>
                                            <button type="button" class="btn shadow btn-transition btn-success float-start" ng-click="pageChanged('client', {justWriteUrl : 'clients-excel'})">
                                            ...
                                            <button type="reset" class="me-2 btn shadow-sm btn-transition btn-light-dark float-end" ng-click="emptyForm('client', true)">
                                    ...
                                    <table class="m-auto table-striped table table-head-custom table-vertical-center table-head-bg table-borderless table-report text-center">
                                        <thead>
                                            <tr class="bg-primary text-white">
                                                ...
                                                <th style="min-width: 120px">Type client</th>
                                                <th style="min-width: 120px">Telephone</th>
                                                <th style="min-width: 120px">Email</th>
                                                <th style="min-width: 120px">Actif</th>
                                                @if(auth()->user()->can('suppression-client') || auth()->user()->can('modification-client') || auth()->user()->can('creation-client'))
                                        ...
                                        <tbody>
                                            <tr class="" ng-repeat="item in dataPage['clients']">
                                                <td>
                                                ...
                                                <td>
                                                    <span class="text-muted fw-bold text-capitalize">@{{item.type_client.nom}}</span>
                                                </td>
                                                <td>
                                                ...
                                                <td class="pr-0 text-right">
                                                    <div class="menu-leftToRight d-flex align-items-center justify-content-center">
                                                        ...
                                                        <button class="menu-btn-item btn btn-sm btn-danger btn-icon font-size-sm" ng-click="deleteElement('client', item.id)" title="{{ __('customlang.supprimer') }}">
                                                            <i class="flaticon2-trash"></i>
                                                        </button>
                                                        @endif
                                                        @if(auth()->user()->can('modification-client'))
                                                        <button class="menu-btn-item btn btn-sm btn-warning btn-icon font-size-sm" ng-click="showModalUpdate('client', item.id, 'null', 'null')" title="{{ __('customlang.modifier') }}">
                                                            <i class="flaticon2-edit"></i>
                                                        </button>
                                                        <button ng-if="!item.status" class="menu-btn-item btn btn-sm btn-light-success btn-icon font-size-sm" title="{{__('customlang.activation')}}" ng-click="showModalStatutNotif($event, 'client', 1, item, {mode:2, title: 'Activer un client'})">
                                                            <i class="fa fa-thumbs-up"></i>
                                                        </button>
                                                        <button ng-if="item.status" class="menu-btn-item btn btn-sm btn-light-danger btn-icon font-size-sm" title="{{__('customlang.desactivation')}}" ng-click="showModalStatutNotif($event, 'client', 0, item, {mode:2, title: 'Désactiver  un client'})">
                                                            <i class="fa fa-thumbs-down"></i>
                                                        </button>
                                                        @endif
                                                        @if(auth()->user()->can('creation-client'))
                                                        <button class="menu-btn-item btn btn-sm btn-light-primary btn-icon font-size-sm" ng-click="showModalUpdate('client',item.id,{forceChangeForm: false, isClone:true}, 'null')" title="{{ __('customlang.cloner') }}">
                    ...
                    <div class="card p-5 rounded-1">
                        <div class="d-flex justify-content-between align-items-center flex-wrap">
                            <div class="d-flex align-items-center me-3">
                                ...
                                <select class="form-control form-control-sm text-primary fw-bold me-4 border-0 bg-light-primary" style="width: 75px;" ng-model="paginations['client'].entryLimit" ng-change="pageChanged('client')">
