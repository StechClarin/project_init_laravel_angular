...
<h4 class="text-center">FACTURE D'ACHATS</h4>
...
{{-- @if(isset($data['produit'])) --}}
    <h4 class="text-center">
        ...
        <span class="text-uppercase"><b>DATE : </b>{{$data['date_facture_fr']}}</span> -------
...
<div>
    ...
    <table class="table">
        ...
        @foreach($data['detailFactures'] as $dt) 
            <tr>
                <td class="td text-left">{{$dt['detComm']["produit"]["code"]}}</td>
                <td class="td text-left">{{$dt['detComm']["produit"]["designation_fr"]}}</td>
                <td class="td text-left">{{$dt['detComm']["produit"]["uniteMesure"]["libelle"]}}</td>
                ...
                @if(\App\Models\Outil::isAuthorize())
                <td class="td text-left">{{Prix_en_monetaire($dt["prix_achat_off"])}}</td>
                @endif
                <td class="td text-left">{{Prix_en_monetaire($dt["total"])}}</td>
                @if(\App\Models\Outil::isAuthorize())
                <td class="td text-left">{{Prix_en_monetaire($dt["totaloff"])}}</td>
                @endif
            </tr>  
            @php
                $total += $dt['total']
        ...
        <tr class="tr">
            ...
            @if(\App\Models\Outil::isAuthorize())
            <td class="td">{{Prix_en_monetaire($totaloff)}}</td> 
            @endif
        </tr>
    </table>
